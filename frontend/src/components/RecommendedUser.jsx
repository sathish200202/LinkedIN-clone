...
const RecommendedUser = ({ user }) => {
  ...
  //get the connection satus query
  const { data: connectionStatus, isLoading } = useQuery({
    queryKey: ["connectionStatus", user._id],
    queryFn: () => axiosInstance.get(`/connections/status/${user._id}`),
  });

  //send the connetion request to other user mutation
  const { mutate: sendConnectionRequest } = useMutation({
    mutationFn: (userId) =>
      axiosInstance.post(`/connections/request/${userId}`),
    onSuccess: () => {
      toast.success("Connection request sent succefffully");
  ...
  //accept the connection request mutation
  const { mutate: acceptRequest } = useMutation({
    ...
    onSuccess: () => {
      ...
      // check if we need to invalidate other queries
    },
    onError: (error) => {
      toast.error(error.response?.data?.error || "An error occured");
    },
  ...
  //reject the connection request mutation
  ...
  //render the different button - connect, accept and reject
  const renderButton = () => {
    if (isLoading) {
      return (
        <button
